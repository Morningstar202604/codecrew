...
// CoTSystemPrompt 返回 Chain-of-Thought 模式的 system prompt 追加部分。
// 要求模型显式展示推理步骤，每步清晰标注。
func CoTSystemPrompt() string {
	return `
## 推理格式（Chain-of-Thought）

请使用链式思考方式回答问题，严格遵循以下格式：
...
注意：思考过程要真实，不要跳过关键步骤；如果某步发现错误，明确指出并修正。
`
}
